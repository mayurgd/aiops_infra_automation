...
function showAgentQuestion(prompt) {
    // Parse HTML content from the prompt if it exists
    let questionContent = prompt;
    
    // Extract the actual question text from the formatted HTML if present
    if (prompt.includes('<div style="background: white;')) {
        const parser = new DOMParser();
        const doc = parser.parseFromString(prompt, 'text/html');
        const questionDiv = doc.querySelector('div[style*="background: white"]');
        if (questionDiv) {
            questionContent = questionDiv.textContent.trim();
        }
    }
    ...
    const questionMessage = `${questionContent}`;
    addMessage('agent', questionMessage, '🤖');
